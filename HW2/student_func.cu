...
__constant__ float const_filter[121];

__global__
void gaussian_blur_RGBA(const uchar4* const inputChannel,
                   uchar4* const outputChannel,
                   int numRows, int numCols,
                   const float* const filter, const int filterWidth)
{
    //for(int i = threadIdx.y * blockDim.x + threadIdx.x; i < local_RGBA_size; i += blockDim.x * blockDim.y) {
    ...
    float resultR = 0.f, resultG = 0.f, resultB = 0.f;
      for (int filter_r = -filterWidth/2; filter_r <= filterWidth/2; ++filter_r) {
        for (int filter_c = -filterWidth/2; filter_c <= filterWidth/2; ++filter_c) {
          ...
          uchar4 u =  inputChannel[image_r * numCols + image_c]; 
            
          float image_value_R = u.x;
          float image_value_G = u.y;
          float image_value_B = u.z;
          float filter_value = const_filter[(filter_r + filterWidth/2) * filterWidth + filter_c + filterWidth/2];

            resultR += image_value_R * filter_value;
            resultG += image_value_G * filter_value;
            resultB += image_value_B * filter_value;
        }
      }
    
    outputChannel[thread_1D_pos] = make_uchar4(resultR, resultG, resultB, 255);
}
                        ...
                        const int filterWidth)
{
  ...
  const dim3 gridSize((blockSize.x + numCols - 1)/blockSize.x, (blockSize.y + numRows - 1)/blockSize.y, 1);
  
  gaussian_blur_RGBA<<<gridSize, blockSize>>>(d_inputImageRGBA, d_outputImageRGBA,  numRows, numCols, d_filter, filterWidth);
...
//makee sure you free any arrays that you allocated
void cleanup() {
